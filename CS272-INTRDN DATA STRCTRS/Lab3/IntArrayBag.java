...
public class IntArrayBag implements Cloneable
{
   ...
   **/   
   ...
   /**
    * Returns true if and array has the same elements and amount of them. Regardless of order
    * @param object
    *   the bag to compare the calling object with
    * @return
    *   True if obj has exact same elements of the same
    *   amount as the calling object. Method disregards order 
    *   as a factor. Returns false if theres a diferrent amount
    *   of an element or element doesnt exist in the other. 
    **/
   public boolean equals(Object obj)
   {
	   if (obj instanceof IntArrayBag) {
		   ...
		   if (candidate.manyItems != manyItems) {
			   return false;
		   }
		   for (int i = 0; i < manyItems; i++) {
			   for (int j = 0; j < manyItems; j++) {
				   if(candidate.data[i] == data[j]) {
					   candidate.remove(data[i]);
					   j = manyItems;
				   }
			   }
		   }
		   if (candidate.manyItems == 0 ) {
			   return true;
		   }
	   }
	   return false;
   ...
   /**
    * Returns a string with its contents between two curly braces
    * @return
    *   A string variable with the calling objects contents
    *   with these contents being place between 2 curly brackets
    **/
   ...
   /**
    * Remove all specified elements from this bag that are in removed.
    * @param removed
    *   the elements to remove from the bag
    * @postcondition
    *   If target was found in the bag, then one copy of
    *   target has been removed and the method returns true. 
    *   Otherwise the bag remains unchanged and the method returns false. 
